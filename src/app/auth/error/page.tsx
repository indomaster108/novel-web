...
export default async function AuthErrorPage({ searchParams }: { searchParams: Promise<{ reason?: string }> }) {
  ...
  const message = reason === "configuration"
    ...
    : "Kode atau tautan sudah kedaluwarsa, tidak valid, atau telah digunakan.";
  return (
    <section className="mx-auto max-w-xl px-4 py-10 sm:px-6 sm:py-20">
      <div className="rounded-[1.75rem] border border-[var(--border)] bg-[var(--surface)] p-6 text-center shadow-[var(--shadow)] sm:p-9">
        ...
        <h1 className="mt-2 font-[family-name:var(--font-lora)] text-3xl font-bold">Verifikasi belum dapat diproses.</h1>
        <p className="mx-auto mt-4 max-w-md text-sm leading-6 text-[var(--muted)]">{message} Gunakan kode dari email terbaru bila pernah meminta kirim ulang.</p>
        {reason !== "configuration" && <ResendConfirmationForm configured={Boolean(getSupabaseConfig())} />}
        <div className="mt-5 flex flex-wrap justify-center gap-x-5 gap-y-2 text-sm font-bold text-[var(--accent)]">
          {reason !== "configuration" && <Link href="/auth/verify" className="inline-flex min-h-11 items-center hover:underline">Masukkan kode verifikasi</Link>}
          <Link href="/auth/login" className="inline-flex min-h-11 items-center hover:underline">Kembali ke halaman masuk</Link>
        </div>
      </div>
    </section>
  );
